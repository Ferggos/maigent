...
SYSTEM_MONITOR_EXTERNAL_DISCOVERY_MIN_CPU_CORES="${SYSTEM_MONITOR_EXTERNAL_DISCOVERY_MIN_CPU_CORES:-0.10}"
SYSTEM_MONITOR_EXTERNAL_DISCOVERY_MIN_RELEVANT_CYCLES="${SYSTEM_MONITOR_EXTERNAL_DISCOVERY_MIN_RELEVANT_CYCLES:-2}"
SYSTEM_MONITOR_EXTERNAL_DISCOVERY_INCLUDE_ROOT_PROCESSES="${SYSTEM_MONITOR_EXTERNAL_DISCOVERY_INCLUDE_ROOT_PROCESSES:-0}"
...
SYSTEM_MONITOR_ARGS=(
  ...
  --external-discovery-min-cpu-cores "${SYSTEM_MONITOR_EXTERNAL_DISCOVERY_MIN_CPU_CORES}"
  --external-discovery-min-relevant-cycles "${SYSTEM_MONITOR_EXTERNAL_DISCOVERY_MIN_RELEVANT_CYCLES}"
)
if [[ "${SYSTEM_MONITOR_AUTO_DISCOVER_EXTERNAL_PROCESSES}" == "1" ]]; then
  SYSTEM_MONITOR_ARGS+=(--auto-discover-external-processes)
fi
if [[ "${SYSTEM_MONITOR_EXTERNAL_DISCOVERY_INCLUDE_ROOT_PROCESSES}" == "1" ]]; then
  SYSTEM_MONITOR_ARGS+=(--external-discovery-include-root-processes)
fi
